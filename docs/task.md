...
Оценивает, насколько высоко в предложенном списке из 20 позиций находятся релевантные (прочитанные пользователем) книги. Метрика учитывает порядок: чем выше релевантный элемент, тем больше вклад.

Для каждого пользователя $u$ рассчитывается:

$$
\mathrm{DCG@20}(u) = \sum_{i=1}^{20} \frac{rel(i)}{\log_2(i+1)}
$$
...
Показывает, какую долю скрытых прочитанных книг ($K$ штук) удалось найти и поместить в топ-20 рекомендованных.

Для каждого пользователя $u$ рассчитывается:

$$
\mathrm{Recall@20}(u) = \frac{|R(u) \cap GT(u)|}{K}
$$
...
- $R(u)$ — ваш ранжированный список из 20 книг для пользователя $u$.
